        ...
        <div style="margin-left: 97%;;">
          ...
          <div class="card">
            <div class="card-body">
              ...
              <!-- Multi Columns Form -->
              <form class="row g-3" action="<?php echo site_url('c_mavericks/createActionJK') ?>"method="post">
                 
                <div class="col-md-6">
                  <label for="inputZip" class="form-label">Bulan</label>
                  <select class="form-select" id="id_bln" name="id_bln"></select>
                </div>
                <div class="col-md-6">
                ...
                <div class="col-6">
                  ...
                  <select class="form-select" id="id_ch" name="id_ch"></select>
                </div>
                <div class="col-md-6">
                  <label for="inputZip" class="form-label">Jenis Kelamin</label>
                  <select class="form-select" id="id_jk" name="id_jk">
                ...
                <div class="col-md-6">
                  ...
                  <input type="text" class="form-control" id="jml_ch" name="jml_ch">
                </div>
...
$(document).ready(function() {
    $.ajax({
        url: '<?php echo site_url("c_mavericks/getajaxbln"); ?>',
        method: 'GET',
        dataType: 'json',
        success: function(response) {
            for (var i = 0; i < response.length; i++) {
                $("#id_bln").append('<option value="' + response[i].id_bulan + '">' + response[i].nama_bulan + '</option>');
            }
        },
    });
});
...
$(document).ready(function() {
    $.ajax({
        url: '<?php echo site_url("c_mavericks/getajaxch"); ?>',
        method: 'GET',
        dataType: 'json',
        success: function(response) {
            for (var i = 0; i < response.length; i++) {
                $("#id_ch").append('<option value="' + response[i].id_curah_hujan + '">' + response[i].tingkat_ch + '</option>');
            }
        },
    });
});
